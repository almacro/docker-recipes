...
# process downloaded CSV files and save new tuples to database
...
def obtain_last_price_dt_saved(symbol_id):
    """
    Obtains datetime of last price for ticker stored in database
    """
    query_str = """SELECT a.id, a.price_date
                   FROM daily_price a
                   INNER JOIN (
                       SELECT p.id, MAX(p.price_date)
                       FROM daily_price p, symbol s
                       WHERE p.symbol_id = s.id
                       AND s.id = %d
                   ) b WHERE a.id = b.id""" % symbol_id
...
def insert_daily_data_into_db(data_vendor_id, symbol_id, daily_data):
    ...
    # Create the time now
    now = datetime.utcnow()
    last_saved_date = obtain_last_price_dt_saved(symbol_id)

    if last_saved_date is ():
        # nothing to do for this ticker
        return
    
    # Amend the data to include the vendor ID and symbol ID
    # and save only new daily price tuples
    daily_data = [
        (data_vendor_id, symbol_id, d[0], now, now,
         d[1], d[2], d[3], d[4], d[5], d[6])
        for d in daily_data
        if d[0] > last_saved_date
    ]
